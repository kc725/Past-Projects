...
(**The possible commands a user can input. If the user's input does not match one
   of these, an exception is thrown.*)
type command =
  | Quit
  | Continue
  | Upgrade
...
(** [disease_parse str disease] parses the string [str] given through a read_line 
    and either changes one of the [disease] fields and returns the new disease, exits the
    disease menu, or raises an error if the input is not formatted correctly.*)
...
(**[parse str] takes the string [str] given from a read_line and returns what kind
   of command the user wishes to run.*)
