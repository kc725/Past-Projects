...
bool coalesce(void *heap) {
	...
	void *start = ADD_BYTES(block_header, (int)sizeof(block_info_t));
	void *next = ADD_BYTES(start, block_header->block_size);
...
void *hl_alloc(void *heap, unsigned int block_size) {
	...
	block_info_t *minblockheader;
	void *minblockstart;
	int minblocksize = heap_header->heap_size + 1;
	bool found = false;
	...
	while (num_blocks_visited < heap_header->num_blocks) {
		...
		if (block_header->free == true && block_header->block_size >= block_size) {
			if (block_header->block_size < minblocksize) {
				minblockheader = block_header;
				minblockstart = start;
				minblocksize = block_header->block_size;
			}
			found = true;
		}
		num_blocks_visited++;
	...
	if (found) {
		//allocate 
		int newblocksize = minblockheader->block_size - (block_size + sizeof(block_info_t) + padding(ADD_BYTES(minblockstart, block_size), sizeof(block_info_t)));
		if (newblocksize > 0) {
			block_info_t *newblock = (block_info_t*)ADD_BYTES(minblockstart, block_size);
			newblock->free = true;
			newblock->block_size = newblocksize;
			minblockheader->block_size = block_size + padding(newblock, sizeof(block_info_t));
			move_block_header(newblock);
			heap_header->num_blocks++;
		}
		minblockheader->free = false;
		return minblockstart;
	}
	//do nothing
	return 0;    
}
...
void hl_release(void *heap, void *block) {
	if (block != 0) {
		...
		block_header->free = true;
		bool combine = coalesce(heap);
		while (combine) {
			combine = coalesce(heap);
		}
	}
}
...
void *hl_resize(void *heap, void *block, unsigned int new_size) {
	...
	block_info_t *block_header = (block_info_t*)ADD_BYTES(block, -sizeof(block_info_t));
	if (block_header->block_size != new_size) {
		block_header->free = true;
